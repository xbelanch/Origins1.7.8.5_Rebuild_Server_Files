...
[] execVM "Scripts\a2edc_buildIdOverlay.sqf";
diag_log format ["A2EDC:REJOIN:CLIENT_INIT phase=pre_variables uid=%1 player=%2 isNullPlayer=%3",getPlayerUID player,player,isNull player];
dayzPlayerLogin = [];
dayzOriginsPlL2 = [];
dayzLoginRecord = nil;
DOdowndblink = nil;
A2EDC_adminTeleportApply = nil;
	"A2EDC_adminTeleportApply" addPublicVariableEventHandler {
		(_this select 1) execVM "admintools\tools\Teleport\a2edc_adminTeleportApply.sqf";
	};
A2EDC_adminHumanityApply = nil;
	"A2EDC_adminHumanityApply" addPublicVariableEventHandler {
		(_this select 1) execVM "admintools\tools\a2edc_adminHumanityApply.sqf";
	};
	diag_log format ["A2EDC:REJOIN:PV_HANDLERS_REGISTERED uid=%1 player=%2 teleportApply=true humanityApply=true",getPlayerUID player,player];
...
diag_log format ["A2EDC:REJOIN:CLIENT_INIT phase=post_variables uid=%1 dayzPlayerLogin=%2 dayzOriginsPlL2=%3 dayzLoginRecordNil=%4 preload=%5",getPlayerUID player,dayzPlayerLogin,dayzOriginsPlL2,isNil "dayzLoginRecord",if (isNil "dayz_preloadFinished") then {"<nil>"} else {dayz_preloadFinished}];


	if (!isServer) then {
		private["_a2edcWaitStart","_a2edcWaitLoops","_a2edcWaitUid","_a2edcWaitReady"];
		_a2edcWaitStart = time;
		_a2edcWaitLoops = 0;
		diag_log format ["A2EDC:REJOIN:CLIENT_WAIT_PLAYER begin uid=%1 player=%2 isNullPlayer=%3",getPlayerUID player,player,isNull player];
		waitUntil {
			_a2edcWaitLoops = _a2edcWaitLoops + 1;
			_a2edcWaitUid = if (isNull player) then {""} else {getPlayerUID player};
			_a2edcWaitReady = (!isNull player) && {_a2edcWaitUid != ""} && {player == player};
			if ((_a2edcWaitLoops == 1) || {(_a2edcWaitLoops % 20) == 0}) then {
				diag_log format ["A2EDC:REJOIN:CLIENT_WAIT_PLAYER retry loops=%1 elapsed=%2 uid=%3 player=%4 isNullPlayer=%5 ready=%6",_a2edcWaitLoops,time - _a2edcWaitStart,_a2edcWaitUid,player,isNull player,_a2edcWaitReady];
			};
			_a2edcWaitReady || {(time - _a2edcWaitStart) > 60}
		};
		_a2edcWaitUid = if (isNull player) then {""} else {getPlayerUID player};
		if ((!isNull player) && {_a2edcWaitUid != ""} && {player == player}) then {
			diag_log format ["A2EDC:REJOIN:CLIENT_WAIT_PLAYER ready elapsed=%1 uid=%2 player=%3",time - _a2edcWaitStart,_a2edcWaitUid,player];
		} else {
			diag_log format ["A2EDC:REJOIN:CLIENT_WAIT_PLAYER timeout elapsed=%1 uid=%2 player=%3 isNullPlayer=%4",time - _a2edcWaitStart,_a2edcWaitUid,player,isNull player];
		};
	};
...
if (!isDedicated) then {
		...
		diag_log format ["A2EDC:REJOIN:LOGIN_REQUEST_SENT gate=pre_monitor uid=%1 player=%2",getPlayerUID player,player];
		_playerMonitor = 	[] execVM "Scripts\a2edc_player_monitor.sqf";
