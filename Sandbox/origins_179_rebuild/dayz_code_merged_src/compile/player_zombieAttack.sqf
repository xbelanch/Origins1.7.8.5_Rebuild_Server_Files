private["_unit","_vehicle","_isBossZ","_targets","_move","_rnd","_wound","_dir","_hpList","_hp","_damage","_chance","_strH","_dam","_total","_result","_tPos","_zPos","_inAngle","_cantSee","_isZombieInside","_building","_isPlayerInside"];
_unit = _this;
_vehicle = (vehicle player);

_isBossZ = _unit isKindOf "z_Boss_zed";
...
/*
	...
	_total = (_dam + _damage);
		...
		//dayHitV =	[_vehicle,_wound,_total, _unit,"zombie"];
		//publicVariable "dayHitV";
		...
		if (_total >= 1) then {
			...
			if(_isBossZ) then {
				...
				diag_log ("1START DAM: Player Hit on " + _wound + " for " + str(_damage));
			} else {
		...
		if (_inAngle) then {
			...
			if (!_cantSee) then {
				...
				if(_isBossZ) then {
					...
					_wound = DAYZ_woundHit call BIS_fnc_selectRandomWeighted;
					//diag_log ("2START DAM: Player Hit on " + _wound + " for " + str(_damage));
				} else {
					_damage = 0.1 + random (1.2);
				};
