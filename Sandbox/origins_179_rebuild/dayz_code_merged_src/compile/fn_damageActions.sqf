...
************************************************************/
private["_menClose","_unit","_unconscious","_lowBlood","_injured","_inPain","_hasBandage","_hasEpi","_hasMorphine","_hasBlood","_action1","_action2","_action","_vehClose","_hasVehicle","_vehicle","_inVehicle","_crew","_unconscious_crew","_patients","_hasJerryB"];
...
if (_hasPatient and !r_drag_sqf and !r_action and !_inVehicle and !r_player_unconscious and _isClose) then {
	...
	_hasJerryB = 	"ItemJerrycanB" in magazines player;
	_hasJerryE = 	"ItemJerrycanEmpty" in magazines player;
	_hasJerryBE = 	"ItemJerrycanBEmpty" in magazines player;
	...
	if(_unconscious) then {
		r_action = true;
		_action1 = _unit addAction [localize "str_actions_medical_02", "\z\addons\dayz_code\medical\pulse.sqf",_unit, 0, true, true];
	...
	*/
	...
	if ((_unit isKindOf "AllVehicles") and !(_unit isKindOf "Man")) then {
		...
		if ((fuel _unit < 1) and (_hasJerry or _hasJerryB)) then {
			r_action = true;
			_action = _unit addAction [format[localize "str_actions_medical_10",_typeVeh], "\z\addons\dayz_code\actions\refuel.sqf",[_unit], 0, true, true, "", "(('ItemJerrycan' in magazines player) or ('ItemJerrycanB' in magazines player))"];
			r_player_actions set [count r_player_actions,_action];
		};
		//CAN WE TAKE FUEL THE OBJECT?
		if ((fuel _unit > 0) and (_hasJerryE or _hasJerryBE)) then {
			r_action = true;
			_action = _unit addAction [format[localize "str_actions_medical_18",_typeVeh], "\z\addons\dayz_code\actions\tfuel.sqf",[_unit], 0, true, true, "", "(('ItemJerrycanEmpty' in magazines player) or ('ItemJerrycanBEmpty' in magazines player))"];
			r_player_actions set [count r_player_actions,_action];
		};
