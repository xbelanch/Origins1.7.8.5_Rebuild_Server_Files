private["_position","_doLoiter","_unitTypes","_isNoone","_loot","_array","_agent","_type","_radius","_method","_nearByPlayer","_attempt","_myDest","_newDest","_lootType","_rndc","_lootBlueAr","_lootBlue"];
...
_lootBlueAr = ["ItemBpt_b1","ItemBpt_b2","ItemBpt_b3","ItemBpt_h1","ItemBpt_h2","ItemBpt_h3","ItemBpt_g_s","ItemBpt_g_b"];
_lootBlue = "";
...
_zombieboss = ({alive _x} count allMissionObjects "z_Boss_zed");
if (_zombieboss==0) then {
	_agent = createAgent ["z_Boss_zed", _position, [], 20, _method];
	//diag_log ("Spawned: " + str(["z_Boss_zed", _position, [], 20, _method]));
	_type = "z_Boss_zed";
} else {
	...
	if (isText _lootType) then {
		...
		if (count _array > 0) then {
			...
			if(!isNil "_array") then {
				if(_type=="z_Boss_zed") then {
					_agent addWeapon _loot;
					_lootBlue = _lootBlueAr select (round(random ((count _lootBlueAr) - 1)));
					_agent addMagazine _lootBlue;
				} else {
